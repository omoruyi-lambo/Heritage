...
import { Question } from '../src/data/store'
      ...
      <header className="bg-white border-b border-gray-200 sticky top-0 z-50">
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <Button variant={viewMode === 'overview' ? 'default' : 'ghost'} onClick={() => setViewMode('overview')} size="sm">Overview</Button>
            <Button variant={viewMode === 'review' ? 'default' : 'ghost'} onClick={() => setViewMode('review')} size="sm">Review Answers</Button>
          </div>
        </div>
      </header>
        ...
        {viewMode === 'overview' ? (
          <div className="space-y-8">
            <div className="bg-white rounded-3xl shadow-sm border border-gray-200 overflow-hidden">
              <div className="p-12 text-center bg-gradient-to-br from-emerald-50 to-white">
                ...
                <h2 className="text-3xl font-bold text-gray-900 mt-6">{isPass ? 'Excellent Work!' : 'Keep Practicing!'}</h2>
                <p className="text-lg text-gray-500 mt-2">You scored {correctCount} out of {questions.length}</p>
              </div>
            ...
            <div className="flex gap-4 justify-center">
              <Button size="lg" onClick={() => setViewMode('review')}>Review Answers</Button>
              <Button size="lg" variant="secondary" onClick={() => router.push('/dashboard')}>Try Another Exam</Button>
            </div>
          </div>
        ) : (
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
            <div className="p-6 border-b border-gray-100 flex justify-between items-center">
              ...
              <div className="flex gap-2">
                <Button variant="secondary" disabled={reviewIndex === 0} onClick={() => setReviewIndex(p => Math.max(0, p - 1))}>Previous</Button>
                <Button variant="secondary" disabled={reviewIndex === questions.length - 1} onClick={() => setReviewIndex(p => Math.min(questions.length - 1, p + 1))}>Next</Button>
              </div>
            </div>

            {currentReviewQuestion && (
              <div className="p-8">
                <div className={`inline-block px-3 py-1 rounded-full text-sm font-medium mb-4 ${isCorrect ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>{isCorrect ? 'Correct!' : 'Incorrect'}</div>
                ...
                <div className="space-y-3">
                  {currentReviewQuestion.options.map((opt, idx) => {
                    ...
                    if (idx === currentReviewQuestion.correctAnswer) style = 'border-emerald-500 bg-emerald-50 text-emerald-900'
                    else if (idx === userAnswerIndex && idx !== currentReviewQuestion.correctAnswer) style = 'border-red-500 bg-red-50 text-red-900'

                    return (
                      <div key={idx} className={`p-4 rounded-xl flex items-center gap-4 ${style}`}>
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold ${idx === currentReviewQuestion.correctAnswer ? 'bg-emerald-500 text-white' : 'bg-white text-gray-400 border border-current'}`}>
